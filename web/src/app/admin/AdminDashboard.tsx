...
export type AdminTab = "overview" | "users" | "subscriptions" | "system";
...
export function AdminDashboard({ initialTab }: { initialTab: AdminTab }) {
  ...
  const filteredUsers = useMemo(() => {
    ...
    const hasDateFilter = joinedDateFilter.length > 0;
    ...
    return managedUsers.filter((entry) => {
      ...
      return dateFilterFormatter.format(createdAtDate) === joinedDateFilter;
    });
  }, [joinedDateFilter, managedUsers, userSearch]);
              ...
              <CardHeader className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                ...
                <div className="flex w-full flex-col gap-3 sm:flex-row sm:items-end sm:justify-end">
                  ...
                  <div className="flex w-full flex-col gap-1 sm:w-48">
                    <Label
                      htmlFor="admin-user-joined-date"
                      className="text-xs font-semibold uppercase tracking-wide text-muted-foreground"
                    >
                      Joined date
                    </Label>
                    <Input
                      id="admin-user-joined-date"
                      type="date"
                      value={joinedDateFilter}
                      onChange={(event) => setJoinedDateFilter(event.target.value)}
                      className="bg-background/80"
                    />
                  </div>
                  {joinedDateFilter ? (
